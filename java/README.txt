...
java_multiple_files={true,false} (command line)
  ...
  contain file-scope enum constants, or if the source file
  has no file-scope enums, the outer class will not be
  generated at all. Unlike java_out which still needs the
  outer class for descriptors and initialization code, and
  therefore forbids a file-scope message name to clash
  with the outer class name, javamicro_out lets you
  'reclaim' the name of the unneeded outer class for one
  of the file-scope messages. NOTE: when overriding this
  option from the command line, all source files and their
  imported files are considered to have the overriding
  value. This may change the fully qualified names of the
