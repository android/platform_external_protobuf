...
java_multiple_files={true,false}:
  ...
  option in all source files and their imported files,
  affecting where classes are placed and how the protocol
  buffer entities are referenced in the Java files.
...
java_multiple_files={true,false} (command line)
  Whether the file should be compiled into multiple
  Java files in the same Java package. If false, which
  is the default, the file will be compiled into one
  Java file, which is the outer class, nesting all the
  classes and constants generated from file-scope
  messages and enums. If true, all file-scope messages
  are compiled into top-level classes, i.e. siblings of
  the file's outer class, in separate files. The outer
  class will only contain file-scope enum constants, or
  if the source file has no file-scope enums, the outer
  class will not be generated at all, in which case the
  outer class name can be used by a file-scope message
  instead. NOTE: when overriding this option from the
  command line, all source files and their imported
  files are considered to have the overriding value.
  This may change the fully qualified names of the
  references to the imported messages and enum values.
...
If there is no file-scope enums, the resulting outer class
will be empty, because unlike java_out, javamicro_out does
not generate descriptors or initialization code in the
outer class. As a result, javamicro_out chooses not to
create the outer class at all. A side benefit of this is
that you can "reclaim" its name for one of the file-scope
messages. For example, the following file:

src/test/proto/one-message.proto:
...
will have an implicit outer class name "OneMessage", which
is also used by the message, but compiles fine with
javamicro_out because the outer class OneMessage would have
been empty and is therefore skipped. The output is as
follows:

com/example/OneMessage.java:

    package com.example;
    public final class OneMessage
        extends MessageMicro {
      ...
    }
